...
export class OrderEffects {
  ...
  public seoTitleUpdateAtOrder$ = createEffect(() =>
    this.actions$.pipe(
      ofType(fromOrderActions.orderSuccess),
      tap(action => {
        this.title.setTitle(SEO_TITLE(`zamówienie numer ${action.orderStore.number}`));
      }),
      mergeMap(() => EMPTY)
    )
  );
  ...
  public createOrderSuccess$ = createEffect(() =>
    this.actions$.pipe(
      ...
      tap(action => {
        ...
        this.rootService.isPlatformBrowser && this.rootService.rrShopWindow.scrollTo(0, 0);
      }),
      switchMap(() => [
  ...
  public seoTitleUpdateAtPotentialOrder$ = createEffect(() =>
    this.actions$.pipe(
      ofType(fromRouterActions.customRouterNavigated),
      concatMap(action => of(action).pipe(withLatestFrom(this.orderFacadeService.isOnPotentialOrderRoute$))),
      filter(([action, isOnPotentialOrderRoute]): boolean => isOnPotentialOrderRoute),
      tap(() => {
        this.title.setTitle(SEO_TITLE('Twój koszyk'));
      }),
      mergeMap(() => EMPTY)
    )
  );
